...
import { DATA_TYPE_LABELS } from '../constants/resourceConstants'
...
const DataResourceLibraryPage: React.FC = () => {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      if (data.items.length === 0) {
        setSelectedId(null)
        setDrawerOpen(false)
      } else if (selectedId && !data.items.some((r) => r.resource_id === selectedId)) {
        setSelectedId(null)
        setDrawerOpen(false)
      }
    } catch (e) {
  ...
  const handlePrevious = () => {
    const currentIndex = items.findIndex((item) => item.resource_id === selectedId)
    if (currentIndex > 0) {
      setSelectedId(items[currentIndex - 1].resource_id)
      setMode('view')
    }
  }
  ...
  const handleDeleteFromDrawer = async () => {
    ...
    try {
      await deleteDataResource(selectedItem.resource_id)
      showSuccess('删除成功')
      setDrawerOpen(false)
      setSelectedId(null)
      fetchList()
              ...
              {items.map((item) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => handleCardClick(item.resource_id)}
                    style={{
                      borderRadius: 8,
                  ...
                  >
                    <div style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
                      <DatabaseOutlined style={{ color: '#faad14', flexShrink: 0, marginTop: 2 }} />
              ...
              <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
                {isViewMode && selectedItem ? (
                  <Descriptions column={1} labelStyle={{ width: 100, fontWeight: 500, color: '#374151' }} contentStyle={{ color: '#1f2937' }}>
                    ...
                    <Descriptions.Item label="类型">{selectedItem.type ? (DATA_TYPE_LABELS[selectedItem.type] || selectedItem.type) : '-'}</Descriptions.Item>
                    <Descriptions.Item label="物理位置">
                      <code style={{ padding: '2px 6px', background: '#f3f4f6', borderRadius: 4, fontSize: 12 }}>{selectedItem.location || '-'}</code>
                    </Descriptions.Item>
                  ...
                  <Form form={form} layout="vertical">
                    ...
                    <Form.Item label="描述" name="description">
                      <Input.TextArea rows={3} placeholder="请输入数据资源描述" />
                    </Form.Item>
...
export default DataResourceLibraryPage
