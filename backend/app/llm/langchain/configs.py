...
def get_opdoc_qa_system_prompt(agent_context: Optional[Dict[str, Any]] = None) -> str:
    ...
    if pre_search_context:
        ...
        # 下方另一种策略：
        # 你只有两个选择：**
        # 1. **意图模糊/多匹配** → 基于预检索信息引导用户明确需求，列出可能相关的模块/功能让用户选择
        # 2. **意图清晰** → 立即调用 `search_yongce_docs`（使用 mix/hybird 模式）进行深度检索并回答
...
**注意** 这是系统内部的信息采集行为，不能让用户感知到你有预查询的动作，严格禁止使用类似 '基于预检索结果xxxx' 的描述内容
**基于上述预检索结果，你必须：
    - 引导用户完善补充一次他的描述，你应该给出选项引导用户做出选择，注意引导应该尽量简洁，聚焦在核心问题而不是事无巨细的让用户选择，最好是几个选项加上一个开放式的反问，比如：或者直接告诉我xxxxx（这个反问的方式可以根据用户问题自己发挥）
    - 如果你发送了引导信息，发送之后必须等待用户返回他的回答，然后你再进行下一步搜索，不能直接搜索，这会导致系统崩溃！
    - 一次引导之后基于用户的答案调用 `search_yongce_docs`（使用 mix/hybird 模式）进行深度检索并回答
    - 例外的是，如果用户的提问非常精准，你结合预查询背景信息已经可以根据用户问题精确锁定某一个业务或者问题，那么你可以直接进入直接搜索工具的流程，不要输出任何类似于：基于预检索结果，我知道了您想了解xxxxx这种解释性文字，直接查询就好
"""
