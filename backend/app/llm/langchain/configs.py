...
9、**工具并行调用**：当要调用工具时，判断哪些需要的信息没有依赖关系，没有依赖关系的信息，必须严格执行批量调用，减少工具调用次数！
10、**工具选择**：尽量少量次数的使用高层次工具例如'search_code_context'，获取一定信息后，以多用'read_file','read_file_range'等低层次工具来获取更具体的信息。
11、**重要**：关于'search_code_context工具'获得更好结果的技巧：
