...
- **图数据库与 LLM**：
  - **Neo4j 同步服务已实现**：`graph_sync_service.py` 提供完整的关系库到图库的同步能力，支持通过 `POST /api/v1/processes/{process_id}/publish` 手动触发同步，或在画布保存时自动同步。
  - **Neo4j 客户端已配置**：`db/neo4j_client.py` 提供 Driver 连接管理，当前连接到云端 Neo4j 实例（neo4j+s://c6010ae0.databases.neo4j.io）。
  - **流程上下文查询**：当前基于关系型数据库聚合（`graph_query_service.get_process_context`），未来可扩展为基于 Neo4j 的图查询实现。
  - **待完善项**：LLM 问答接入、骨架生成服务、配置管理规范化（环境变量）、同步监控与错误处理优化等仍在规划中。
...
**核心函数**：`backend/app/services/graph_sync_service.py` 中的 `sync_process(db: Session, process_id: str) -> None`

**实现逻辑**：

1. 从 SQLite 读取数据：
   - 该 `process_id` 的 `Business` 记录
   - 对应的 `ProcessStepEdge` 列表（流程内的步骤连线）
   - 所有相关的 `Step` / `StepImplementation` / `Implementation` / `ImplementationDataResource` / `DataResource`
2. 在 Neo4j 中执行同步：
   - 清理旧数据：删除该 `process_id` 相关的 `Business` 节点及其关联子图，删除相关的 `NEXT`/`EXECUTED_BY`/`ACCESSES_RESOURCE` 关系
   - 创建约束：确保 `Business.process_id`、`Step.step_id`、`Implementation.impl_id`、`DataResource.resource_id` 的唯一性
   - 同步节点：使用 `MERGE` 语句创建或更新 4 类节点
   - 同步关系：通过拓扑排序确定起始步骤，创建 `START_AT` / `NEXT` / `EXECUTED_BY` / `ACCESSES_RESOURCE` 关系

### 5.2 与现有脚本的一致性

当前实现完全遵循 `test/neo4j_load_open_card.py` 的逻辑：

- 节点属性：`Business`/`Step`/`Implementation`/`DataResource` 的字段与 PoC 脚本完全一致
- 关系类型：`START_AT`/`NEXT`/`EXECUTED_BY`/`ACCESSES_RESOURCE` 与设计文档第 2.3 节定义一致
- 关系属性：`NEXT` 关系携带 `process_id`，`ACCESSES_RESOURCE` 关系携带 `access_type` 和 `access_pattern`

### 5.3 触发策略（已实现）

**手动触发**：
- API 接口：`POST /api/v1/processes/{process_id}/publish`，由 `backend/app/api/v1/processes.py` 提供
- 前端可通过"发布到图谱"按钮调用

**自动触发**：
- 画布保存时同步：`POST /api/v1/canvas/save` 接口在保存画布数据到 SQLite 后，自动调用 `sync_process` 同步到 Neo4j
- 采用异常容错机制：Neo4j 同步失败不影响 SQLite 保存结果，失败时记录 warning 日志

**待完善**：
- 事件驱动同步（基于数据库触发器或消息队列）
- 批量同步接口（同步所有流程）
- 增量同步策略（只同步变更部分）
- 同步状态追踪（记录最后同步时间、同步结果）
- 配置管理规范化（将硬编码的 Neo4j 连接信息改为环境变量）
